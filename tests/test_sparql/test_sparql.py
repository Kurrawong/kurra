...
def test_query_db(fuseki_container, http_client):
    ...
    assert (
        "c"
        in json.loads((render_sparql_result(query(SPARQL_ENDPOINT, q, http_client), RenderFormat.json)))["head"]["vars"]
    )
    ...
    r = query(SPARQL_ENDPOINT, q, http_client, return_python=True, return_bindings_only=True)
    assert r[0]["c"]["value"] == "https://example.com/demo-vocabs/language-test/en-only"

    r = query(SPARQL_ENDPOINT, q, http_client, return_python=True, return_bindings_only=False)
    assert r["results"]["bindings"][0]["c"]["value"] == "https://example.com/demo-vocabs/language-test/en-only"

    r = query(SPARQL_ENDPOINT, q, http_client, return_python=False, return_bindings_only=False)
    assert isinstance(r, str)
    r2 = json.loads(r)
    assert r2["results"]["bindings"][0]["c"]["value"] == "https://example.com/demo-vocabs/language-test/en-only"

    r = query(SPARQL_ENDPOINT, q, http_client, return_python=False, return_bindings_only=True)
    assert isinstance(r, str)
    r2 = json.loads(r)
...
def test_auth(fuseki_container, http_client):
    ...
    with pytest.raises(RuntimeError):
        r = query(SPARQL_ENDPOINT, q, None, return_python=True, return_bindings_only=True)
